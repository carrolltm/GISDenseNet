...
def pred_socre():
    with open('path/to/data.pkl', 'rb') as f:
        rf = cPickle.load(f)
    new_df = pd.read_csv("video.csv").fillna(-1)
    new_df = new_df.drop('filename', axis=1)
    train_count = int(0.7 * len(new_df))
    preds = rf.predict(new_df.iloc[1:, 1:])
    print(preds)

def train_socre():
    # 我们做的输入数据   决策树中属性来判断到达叶子结点就是分数
    # 这个程序做的就是这个每个属性对应的数据，（每个样本里面对应属性的数据） 这里的叶子节点是什么
    df = pd.read_csv("merged.csv").fillna(-1)
    ...
    """
            ...
            random_state:      随机种子，设置之后每次生成的n_estimators个样本集不会变，确保实验可重复
    """
    clf = RandomForestRegression(n_estimators=8,
                                 max_depth=-1,
    ...
    # ref:https://blog.csdn.net/sinat_29675423/article/details/87975489
    # 前train_count行*所有标签列         前train_count行名为label的那一列
    # 将数据和标签填充到随机森林中
    clf.fit(df.iloc[:train_count, :-1], df.loc[:train_count - 1, 'label'])
    from sklearn import metrics

    with open('path/to/data.pkl', 'wb') as f:
        cPickle.dump(clf, f)

        # 返回一个值来,minimize （最小化），越低越好
    # 均方误差回归损失
    # 前train_count训练出的随机森林和label之前的均方误差
    print(metrics.mean_squared_error(df.loc[:train_count - 1, 'label'], clf.predict(df.iloc[:train_count, :-1])))
    # 剩下的没参与训练的作为测试集和训练出的模型之间的误差
    print(metrics.mean_squared_error(df.loc[train_count:, 'label'], clf.predict(df.iloc[train_count:, :])))
